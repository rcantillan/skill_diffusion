...
prepare_all_data <- function(all_skills_data, output_dir = "data_output") {
  ...
  for(period_name in names(network_data)) {
    ...
    # AQUÍ ESTÁ LA MODIFICACIÓN: Usar write.table con separador ";" en lugar de write.csv
    ...
    write.table(
      ...
      dec = ".",              # Usar punto como separador decimal (más compatible)
      quote = TRUE            # Asegurar que los textos estén correctamente citados
    )
...
#skill <- read_exported_csv(here("/home/rober/Documentos/skill_diffusion/skillscape_alabdulkareem_method/", "2005-2009", "skill_rca.csv"))
