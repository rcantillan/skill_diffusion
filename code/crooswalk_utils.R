...
format_soc_code <- function(code, format_type = "standard") {
  if (is.na(code) || is.null(code) || trimws(as.character(code)) == "") return(NA_character_)
  
  # Limpiar el código
  code_clean <- trimws(as.character(code))
  
  if (format_type == "onet") {
    # Para códigos O*NET-SOC (pueden incluir extensiones .00, .01, etc.)
    code_clean <- gsub("[^0-9\\.\\-]", "", code_clean)
    
    # Extraer el código SOC base (XX-XXXX)
    ...
    # Para códigos Census OCC (típicamente 4 dígitos)
    code_clean <- gsub("[^0-9]", "", code_clean)
    if (nchar(code_clean) > 0 && nchar(code_clean) <= 4) {
      return(sprintf("%04d", as.integer(code_clean)))
    }
  } else {
    # Formato SOC estándar (XX-XXXX)
    code_clean <- gsub("[^0-9\\-]", "", code_clean)
...
load_crosswalk <- function(file_path, type, col_specs) {
  ...
  for (col in names(result_dt)) {
    ...
    result_dt[, (col) := sapply(.SD[[col]], format_soc_code, format_type = format_type), .SDcols = col]
  }
